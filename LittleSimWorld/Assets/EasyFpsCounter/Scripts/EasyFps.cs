...
public class EasyFps : MonoBehaviour {
    ...
    private Color _fpsNormalTextColor;

    void Start () {

        txt = transform.Find("Text").GetComponent<TextMeshProUGUI>();
        _fpsNormalTextColor = txt.color;
        
        if (Settings.Display.VSyncOn) txt.color = Color.red;
        
        Settings.Display.onVSyncChanged.AddListener((vsyncOn =>
            {
                txt.color = vsyncOn ? Color.red : _fpsNormalTextColor;
            }));
